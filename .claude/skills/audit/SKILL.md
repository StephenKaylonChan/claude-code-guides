...
stat -f "%Sm" .claude/session-notes.md 2>/dev/null || stat -c "%y" .claude/session-notes.md
```
...
if command -v gitleaks &>/dev/null; then
  ...
  rg -iE "(password|secret|api[_-]?key|token)\s*[:=]\s*['\"][^'\"]+['\"]" \
    *.md .claude/ --glob '!.git' 2>/dev/null | head -10
fi
```

### 5b. 敏感文件检查

- `.claude/settings.local.json` 是否在 `.gitignore`？
- 未跟踪的 `.env*` 或 `*.key` 文件？
...
| 密钥扫描 | 保留，重点扫 `.claude/settings.local.json` 和 markdown 误贴 |
