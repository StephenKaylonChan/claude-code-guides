...
| Bug 审查 | 所有关键源文件 + 最近改动 |
| 代码质量 | 核心业务模块 + 工具函数 |
| 安全审查 | 认证 / 授权 / 输入处理 / 数据库访问（根据项目结构定位） |
...
每个文件标完整路径：
```markdown
### `apps/website-web/src/components/ListPage.tsx`
\`\`\`tsx
[文件内容]
\`\`\`
```
...
total_chars=$(wc -m < .codex-task-draft.md 2>/dev/null || echo 0)
...
project: [从 pwd 或 CLAUDE.md 提取项目名]
scope: [包含范围，如 "全部源文件" / "最近改动" / "apps/website-api/ 模块"]
---
