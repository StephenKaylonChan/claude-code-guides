...
检查全部通过后提交。**MUST NOT 用 `git add -A` / `git add .`**——它会把所有未跟踪文件（可能含本不想提交的本地产物，如 `.idea/`、外部工具配置）一并扫入。

```bash
# 1. 只暂存「已跟踪文件」的修改（不碰未跟踪文件，安全）
git add -u
...
- **未跟踪文件处理**：若 Step 2 列出了未跟踪文件，**逐个判断**是否属于本次改动应提交的新文件（如新建的文档 / skill）：
  - 属于 → 用 `git add <具体路径>` 显式加入（**指明文件名，不用通配**）
  - 不属于（本地产物、外部工具配置、临时文件）→ **不加**，保持未跟踪
  - 拿不准 → 用 AskUserQuestion 列出未跟踪文件让用户确认
- 暂存完成后提交：
