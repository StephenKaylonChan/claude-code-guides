...
name: docs
description: |
  guides 文档生态守护者：跨 00-04 + README + prompt-*.md + .claude/skills/*/SKILL.md 一致性审计与修复。
...
1. 03 Skill 模板（真相源） vs `.claude/skills/*/SKILL.md`（本地定制副本）
2. 02/03 文档 vs 3 个 prompt-*.md（发给用户的指引） 
3. CLAUDE.md 同步表 vs 实际版本/数量引用散布
...
| `skills` | 03 Skill 模板 vs 本地 `.claude/skills/*/SKILL.md` 一致性 |
...
03-Skills命令配置.md 的 2.1-2.12 是**真相源**，本地 `.claude/skills/*/SKILL.md` 可以定制，但 `name` / `description` / `allowed-tools` / `argument-hint` 应保持同步。
...
### 2.3 CLAUDE.md 同步表反查（全量）

读 CLAUDE.md 的"修改时必须同步的内容"表，对表内每项抽样验证：
...
| 通用版 | guides 本地版 |
|--------|---------------|
| "代码-文档一致性" | "源头-副本一致性"（03 Skill vs 本地 SKILL.md；CLAUDE.md 同步表 vs 散布数字）|
