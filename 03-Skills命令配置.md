...
共 14 个自定义 Skill，按用途分为四类：

| 类别 | Skills | 说明 |
|------|--------|------|
| **质量审查** | `/audit`、`/diagnose` | 项目健康浅层巡检、代码架构量化诊断（文档一致性归 `/docs`）|
| **开发流程** | `/catchup`、`/handoff`、`/spec`、`/implement`、`/done`、`/loop-engineering` | 上下文恢复、会话交接、设计文档、单改动实施、收尾检查、目标驱动循环 |
| **文档管理** | `/docs`、`/release` | 开发文档梳理、Phase 系统性刷新 |
| **工具** | `/nbp2`、`/fix-permission`、`/codex`、`/visual` | AI 生图、权限诊断、外部 AI 任务文档、项目数据可视化 |
...
### 2.13 /visual — 项目数据可视化仪表盘

**用途**：把项目数据（roadmap 进度、audit 报告、跨文件变更等）渲染成**自包含 HTML 仪表盘**，替代难以阅读的长 Markdown 报告。

**定位**：**只读可视化工具**——生成一次性 HTML 文件，浏览器打开即用，不修改源文件，不进版本控制。

> **设计理念**：参考 Anthropic 工程师 Thariq Shihipar "HTML is ridiculously effective" 实践——AI 写文件、人看文件的场景下，HTML 的空间布局、色彩编码、交互筛选比纯文本高一个量级。

### v3.35 新增

- **首次写入 03 模板**（v3.34 起在 guides 本地验证）
- **4 种场景**：`roadmap`（进度仪表盘）/ `audit`（报告可视化）/ `changes`（变更地图）/ 自由描述
- **内置暗色设计系统**：CSS 变量色板 + serif 标题 + system-ui 正文，所有输出视觉一致
- **4 种布局模式**：仪表盘（统计卡片 + 进度条 + 筛选）/ 时间线（版本历史）/ 风险地图（P0-P2 色标）/ 变更地图（文件增删色块）
- **零外部依赖**：纯 HTML + inline CSS + vanilla JS
- **输出**：`docs/reports/{场景}-{日期}.html`，自动浏览器打开
- **`.gitignore`**：建议加 `docs/reports/*.html`

### 何时用 /visual vs 保留 Markdown

| 场景 | 选择 |
|------|------|
| 人看一眼做决策的报告 | **`/visual`** |
| Claude 要读回去的文件（CLAUDE.md、spec、session-notes） | Markdown |
| 空间对比、进度可视化、分享给团队 | **`/visual`** |
| 需要版本控制 diff 的长期文档 | Markdown |

### 和其他 skill 的关系

| Skill | 关系 |
|-------|------|
| `/audit` | `/audit` 生成 Markdown → `/visual audit` 渲染为 HTML |
| `/diagnose` | 13 维度评分适合仪表盘布局 |
| `/catchup` | 文字恢复上下文 → `/visual roadmap` 补充视觉概览 |
| `/release` | Phase 发版后跑 `/visual roadmap` 看全局进度 |

**文件路径**: `.claude/skills/visual/SKILL.md`
...
### 2.14 /loop-engineering — 目标驱动有限循环

**用途**：把用户的目标转成一个有边界的 Agent 工作循环：先定义 Loop Contract，再按 Observe → Decide → Act → Verify → Reflect 迭代。普通步骤自动推进；重大决策、阻塞、预算耗尽、外部副作用或人工 Gate 停下来问用户。

**定位**：**Loop Engineering 方法论的安全落地层**。它不是无限自动执行器，也不替代 `/spec`、`/implement`、`/done`、`/handoff`、`/catchup`；它负责把目标、预算、停止条件、验证 Gate 和人工确认点写清楚，再编排既有工作流持续推进。

> **设计原则**：从 Prompt Engineering（优化单次输入）升级到 Loop Engineering（设计可验证、可停止、可恢复的 Agent 工作系统）。第一版保守：不默认创建后台 routine/automation，不默认预批准写文件，不允许 Agent 自评即完成。

### v3.40 新增

- **Loop Contract**：Goal / Scope / Non-goals / Mode / Budget / Done condition / Stop condition / Block condition / Verification gate / Human confirmation points
- **有限迭代**：默认最多 3 轮、最多 1 次自动返工、上下文超过 60%-70% 建议 `/handoff`
- **自动推进边界**：普通只读检查、拆分 iteration、低风险验证自动推进；目标不清、范围扩大、外部副作用、人工 Gate、预算耗尽 MUST 停下来
- **既有 Skill 编排**：复杂设计先 `/spec`；单点执行交给 `/implement`；完成验收交给 `/done`；中断恢复用 `/handoff` / `/catchup`
- **独立审查**：高风险或大搜索空间时使用 subagent / reviewer，避免实现 Agent 自我确认
- **保守权限**：`disable-model-invocation: true`，`allowed-tools` 只读 + `disallowed-tools: Write, Edit` 硬禁止写入（写入一律走 `/implement`）
- **前置闸门**：开 loop 前先判断"要不要 loop"——稳定可重复的任务用脚本 / cron / lint（distill and demote），简单"跑到完成 / 周期触发"用内置 `/goal` / `/loop` 即可，不重复造轮子
- **停滞 circuit-breaker**：连续 2 轮陷入局部最优 → 停下换更大胆改动或问用户
- **完成判定独立**：Done 由 Gate 或独立 reviewer 判，MUST NOT 自评完成
- **自治分级**：L1 仅报告 / L2 协助修复 / L3 无人值守（首版不鼓励直接 L3）
...
| `/implement` | 每个 Act 阶段 SHOULD 降解为一个 `/implement` 可处理的单改动 |
...
| `/audit` / `/docs` / `/codex` | 可作为 readonly、文档一致性或外部审查阶段的工具 |

**文件路径**: `.claude/skills/loop-engineering/SKILL.md`
...
- **任务稳定、可重复、规则明确** → 用脚本 / cron / 查询 / lint 规则 / 模板，不要套 LLM loop（社区称 "distill and demote"：把可预测行为下沉为确定性代码）。
- **只是简单的"跑到完成"或"周期触发"** → 内置 `/goal`（设完成条件后持续推进）或 `/loop`（定时 / 自定步调重复）可能就够了。本 Skill 用在需要**显式 Loop Contract + 验证 Gate + maker/checker 纪律**的场景，**不替代**这两个内置命令。
...
- 达到预算上限
- 连续 N 轮无新证据或无进展
- 连续 2 轮陷入同一状态/局部最优（circuit-breaker）：停下换更大胆的改动方向或问用户
**Block condition**:
- 缺少权限/凭证/环境
- 目标或范围不清
- 修改面扩大到未批准范围
...
> 这套 Observe → Decide → Act → Verify → Reflect 是社区 "Plan-Act-Observe" 循环的超集（多了显式 Verify Gate 和 Reflect 复盘）。卡在局部最优（连续两轮结果相同 / 小抖动）MUST 停下换更大改动或问用户。

进入下一轮前必须检查：是否达到 Done condition、触发 Stop/Block/Human confirmation point、以及是否仍在预算内。
...
**完成判定（Done）MUST 独立**：依据 Verification Gate（command / auto）或独立 reviewer agent，MUST NOT 由推进 loop 的同一上下文自评"做完了"。

以下情况 SHOULD 使用 subagent 或独立 review：改动跨多个模块或多个文档源头、实现者容易自我确认完成、验证面包含安全/权限/数据一致性、搜索空间很大。

规则：任务必须具体、自包含；不把预期答案泄露给 reviewer；实现 agent 与 review agent 职责分离；不默认每轮都开 subagent。
...
- **L2 协助修复**：在明确 scope + Gate 内推进，每个写入走 `/implement`，关键节点人工确认。
- **L3 无人值守**：仅当 Gate 完全确定性、回滚成本低、用户显式授权时才考虑；第一版**不鼓励**直接跳到 L3。

## 与 /diagnose D11 联动（distill and demote）

loop 多轮后若发现**反复犯同一类错 / 反复手工修同一种问题**，SHOULD 把它下沉为确定性规则（ESLint / `.claude/rules/` / 脚本 / Hook），对应 `/diagnose` D11 一致性维度"重复犯错升级为 lint 规则"的理念。
...
/visual roadmap                  # 项目进度可视化仪表盘（HTML）
/visual audit                    # 最近审计报告可视化
/visual changes                  # 当前分支变更地图
/visual                          # 无参数，AskUserQuestion 选场景

/loop-engineering "修完 CI 直到通过" --execute --budget 3   # 有边界的目标循环
/loop-engineering "调研某技术是否适合做 Skill" --readonly   # 只读调研 + Loop Contract
```
