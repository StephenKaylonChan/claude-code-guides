...
- **只是简单的"跑到完成"或"周期触发"** → 用平台原生的循环 / 定时能力（如 Codex Automations、`codex exec` 脚本循环）可能就够了。本 Skill 用在你需要**显式 Loop Contract + 验证 Gate + maker/checker 纪律**的场景。
...
先读取项目约束（优先 `AGENTS.md`）和当前状态，避免在错误上下文中开 loop：
...
rg -n "^### 2\\.[0-9]+ /|Skills 总数|\\.agents/skills" 03-Skills命令配置.md README.md prompt-*.md
```
...
loop 多轮后若发现**反复犯同一类错 / 反复手工修同一种问题**，SHOULD 把它下沉为确定性规则（ESLint / 项目 rules / 脚本 / Hook），对应 `/diagnose` D11 一致性维度"重复犯错升级为 lint 规则"的理念——让下次不再需要 loop。
...
- 和平台原生循环 / 定时能力（Codex Automations / `codex exec`）职责重叠，重复造轮子
