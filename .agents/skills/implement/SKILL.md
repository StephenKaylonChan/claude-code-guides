...
| 改动会让"同步表"里某行变更 | AGENTS.md 同步表指向的是重量级改动 |
...
- 改的内容是否在 AGENTS.md 同步表覆盖范围（版本号/Hook 数/Skills 数/章节编号/新版本功能）？是 → 升级 /spec
...
遵循 guides AGENTS.md 编写规范（MUST/MUST NOT/SHOULD；表格对齐；代码块标注；跨文档引用格式）。
...
打开 AGENTS.md 的"修改时必须同步的内容"表，逐行对照本次改动：**任一行被触及 → 停下来走 /spec 或先讨论**。不要闷头同步。
...
- **AGENTS.md 同步表是硬分流器**：表内项目一旦触发 → 升级 /spec；表外的小改动 → /implement 即可
