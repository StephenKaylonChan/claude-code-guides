...
> 整个放开 `WebFetch` 能消灭所有弹窗，但会拆掉一道真实防线：恶意网页可藏注入指令，诱导 Codex 把敏感数据拼进 URL 抓到外部站（数据外泄——"网页能读 + 能外发 + 可能读到坏内容"三者凑齐）。逐域名白名单正是掐断"外发"这一环。**低敏感 + 有人盯着的场景可接受裸放，但务必让用户清楚拆掉的是什么。**
>
...
> - **此场景裸放 `WebFetch` 是正解，不算"图省事"**：写入用户级 `~/.Codex/settings.json`（裸 `WebFetch` 会覆盖所有 `WebFetch(domain:*)` 行使其变冗余，旧行可留可删）；仍须让用户清楚拆掉的是上面那道"外发"防线，可随时删行还原。
