...
**已修改文件**（`git status --short` 里 ` M` 开头）：Codex 读取并自动 stage。

**新增文件**（`??` 开头，unstaged）：自动 stage，但 **MUST 排除垃圾名单**——`.env*`、`*.log`、`*.tmp`、`node_modules/`、`dist/` / `build/` 等构建产物、`.DS_Store`、以及 `.Codex/session-notes.md` 自身。
...
简单 / 复杂改动**一律由 Codex 直接生成** Conventional Commits message，不再列候选弹窗。
