...
§7.2 讲"怎么手动切"，本节讲"不切时默认走哪个、子代理跟谁走"——理解这套规则才能精准控制成本与质量。

**核心规则：不钉就继承。** Claude Code 解析"这次用哪个模型"时，按优先级从高到低取**第一个有值**的来源：

| 优先级    | 来源                                   | 作用范围              |
|-----------|----------------------------------------|----------------------|
| 1（最高） | `--model` 参数 / 会话内 `/model`        | 本次会话临时          |
| 2         | 项目本地 `settings.local.json` 的 `model` | 当前项目（不进 git）  |
| 3         | 项目级 `settings.json` 的 `model`       | 当前项目（团队共享）  |
| 4         | 用户级 `~/.claude/settings.json` 的 `model` | 所有项目          |
| 5（兜底） | Claude Code 内置默认                    | 全局                  |

> 全程没有任何一层写 `model` 字段时，模型完全由运行时 `/model` 决定，**关闭会话即失效**。要"每次开都固定某模型"，MUST 把 `model` 写进某级 `settings.json`，而非每次手动 `/model`。

**钉模型的三个入口**（粒度从粗到细）：

| 入口                    | 写法                              | 影响范围                  |
|-------------------------|-----------------------------------|--------------------------|
| settings `model` 字段   | `"model": "claude-opus-4-8"`      | 主循环 + 所有继承的子代理 |
| Agent 定义 frontmatter  | `model: claude-sonnet-4-6`        | 仅该类自定义 agent        |
| `Agent` 工具调用参数    | 调用时由模型传入                   | 仅单次调用                |

**子代理模型继承**：子代理（Explore / Plan / 自定义 agent / workflow 子代理）默认**继承主循环模型**——主循环用 Opus，它们就 Opus；`/model` 切 Sonnet，它们跟着切。

> ⚠️ Claude Code **没有**"子代理默认模型"这种独立全局开关。想让主循环留 Opus、子代理降到便宜模型省钱，**唯一**做法是给**自定义** agent 的 frontmatter 逐个写 `model:`（上表入口 2）。内置 Explore / Plan 改不了其 frontmatter，**永远**继承主循环。
>
> `effortLevel` 同样遵循"不钉就继承 + 各级 settings 覆盖"，规则同上表（无 `model`/`effortLevel` 字段 = 运行时自己决定，适合不想锁死的用法）。
